...
function toPublicSettings(settings: Awaited<ReturnType<typeof getOrCreateSettings>>) {
  ...
  return {
    ...
    currencyPosition: general.currencyPosition || 'before',
...
export async function getSettings(_req: Request, res: Response): Promise<void> {
  ...
  const data = toPublicSettings(settings) as Record<string, any>;
  // Maps API key is a public key by design, sourced from the developer-only group.
  data.googleMapsApiKey = ((settings.developerSettings as Record<string, any>) || {}).googleMapsApiKey || null;
...
// ============================================================
// DEVELOPER SETTINGS (DEVELOPER role only — hidden from admins)
// ============================================================

const DEVELOPER_SECRET_FIELDS = ['googleMapsApiKey', 'sentryDsn', 'recaptchaSecretKey'] as const;

export async function getDeveloperSettings(_req: Request, res: Response): Promise<void> {
  const data = await getSettingsGroup('developerSettings');
  res.json({
    success: true,
    data: {
      ...data,
      googleMapsApiKey: maskSecret(data.googleMapsApiKey),
      sentryDsn: maskSecret(data.sentryDsn),
      recaptchaSecretKey: maskSecret(data.recaptchaSecretKey),
    },
  });
}

export async function updateDeveloperSettings(req: Request, res: Response): Promise<void> {
  ...
  const existing = await getSettingsGroup('developerSettings');
  const mergedData: Record<string, any> = { ...parsed.data };
  for (const field of DEVELOPER_SECRET_FIELDS) {
    mergedData[field] = preserveIfMasked(parsed.data[field], existing[field]) ?? null;
  }

  const data = await updateSettingsGroup('developerSettings', mergedData);
  res.json({
    success: true,
    data: {
      ...data,
