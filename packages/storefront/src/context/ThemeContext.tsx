...
export function formatPriceValue(amount: number, symbol = '$', position: 'before' | 'after' = 'before'): string {
...
const ThemeContext = createContext<ThemeContextType>({
  ...
  isDark: false,
  formatPrice: (amount: number) => formatPriceValue(amount),
});
...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  const formatPrice = (amount: number) =>
    formatPriceValue(amount, settings.currencySymbol, settings.currencyPosition);

  return (
    <ThemeContext.Provider value={{ settings, isDark, formatPrice }}>
      {children}
