...
export async function listStaff(req: Request, res: Response): Promise<void> {
  ...
  const where: any = {};
  if (role && ['SUPER_ADMIN', 'MANAGER', 'STAFF', 'DEVELOPER'].includes(role)) {
    where.role = role;
  }
...
const updateStaffSchema = z.object({
  ...
  role: z.enum(['SUPER_ADMIN', 'MANAGER', 'STAFF', 'DEVELOPER']).optional(),
...
// ============================================================
// CREATE STAFF (admin sets email + password, no email sent)
// ============================================================
...
export async function createStaff(req: Request, res: Response): Promise<void> {
  ...
  const { email, password, name, role, phone, locationId } = parsed.data;
  ...
  auditLog(req, { action: 'create', entity: 'Staff', entityId: user.id, details: { email, role: role || 'STAFF' } });
