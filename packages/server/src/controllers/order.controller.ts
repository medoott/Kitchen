...
import { emitNewOrder, emitOrderStatusUpdate, emitTableStatusUpdate, emitTableOrderUpdate, emitTableCartUpdate } from '../lib/socket.js';
...
export async function createOrder(req: Request, res: Response): Promise<void> {
  ...
  if (tableSessionToken) {
    const found = await prisma.tableSession.findUnique({
      where: { sessionToken: tableSessionToken },
      include: { table: true },
    });
    if (!found || found.closedAt) {
      res.status(404).json({ success: false, error: 'Table session not found or closed' });
      return;
    }
    tableSession = found as any;
  }
  const isTableOrder = !!tableSession;
  ...
  // Guest checkout: require name + email if not authenticated (not needed for table orders)
  if (!customerId && !isTableOrder) {
  ...
  // Get location (table orders use the table's location)
  const location = await prisma.location.findFirst({
    where: isTableOrder ? { id: tableSession!.table.locationId } : { isActive: true },
  ...
  if (scheduledAt && !isTableOrder && location.operatingHours.length > 0) {
    const scheduled = new Date(scheduledAt);
  ...
  const order = await prisma.order.create({
    data: {
      ...
      guestName: customerId ? undefined : isTableOrder ? `Table ${tableSession!.table.name}` : guestName,
  ...
  // QR table ordering: mark table as waiting for food and clear the shared cart
  if (isTableOrder && tableSession) {
    const { TableStatus } = await import('@prisma/client');
    const updatedTable = await prisma.table.update({
      where: { id: tableSession.tableId },
      data: { status: TableStatus.WAITING_FOOD },
    });
    emitTableStatusUpdate({ id: updatedTable.id, status: updatedTable.status, name: updatedTable.name });

    await prisma.tableSession.update({
      where: { id: tableSession.id },
      data: { cart: [] as any, status: TableStatus.WAITING_FOOD },
    });
    emitTableCartUpdate(tableSession.tableId, {
      sessionToken: tableSession.sessionToken,
      tableId: tableSession.tableId,
      cart: [],
    });
    emitTableOrderUpdate(tableSession.tableId, {
      id: order.id,
      orderNumber: order.orderNumber,
      status: order.status,
      total: order.total,
